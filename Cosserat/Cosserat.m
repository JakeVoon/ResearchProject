function CantileverRod
    L = 0.4; N = 40; E = 207e9; r = 0.0012;

    rho = 8000; vstar = [0;0;1];

    g = 10*[-9.81;0;0]; %10x greater for effect

    Bse = zeros(3); Bbt = zeros(3); C = zeros(3);

    del_t = 0.005; STEPS = 50;

    F_tip = [0;0;0]; M_tip = [0;0;0];
...
function [ys,z] = ODE(y,yh,zh)
    ...
    R = eye(3) + 2/(h'*h) * [-h3^2-h4^2, h2*h3-h4*h1, h2*h4+h3*h1; h2*h3+h4*h1, -h2^2-h4^2, h3*h4-h2*h1; h2*h4-h3*h1, h3*h4+h2*h1, -h2^2-h3^2];
